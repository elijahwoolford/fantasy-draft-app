...
def display_countdown(duration):
    ...
    st.markdown("""
        <style>
        .good-luck, .countdown-text {
            font-size: 64px;
            font-weight: bold;
            color: #32CD32;  /* LimeGreen for both */
            text-align: center;
    ...
    # Show "Good Luck!" with pulse animation
    placeholder.markdown("<div class='good-luck'>Good Luck!</div>", unsafe_allow_html=True)
    time.sleep(2)

    # Countdown with same style (but no animation)
    for remaining in range(duration, 0, -1):
        placeholder.markdown(f"<div class='countdown-text'>{remaining}</div>", unsafe_allow_html=True)
        time.sleep(1)
...
def main():
    ...
    if st.button("Draw Lottery"):
        if len(teams) == 4 and total_odds == 100:
            input_placeholder.empty()  # 👈 Clear the input section
